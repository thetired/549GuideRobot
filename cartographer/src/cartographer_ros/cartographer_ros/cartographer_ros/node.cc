 ...
 * @param[in] handler 函数指针, 接受传入的函数的地址,Node下的函数指针
      ...
      map_builder_bridge_(node_options_, std::move(map_builder), tf_buffer)
{
  ...
  // 发布SubmapList
  //这些publisher  都是 ros::publisher类型
  //node_handler类的  advertise函数是模板函数，要写参数的类型，
  //两个参数分别是 发布话题的名称，对立中最大报错的消息数
  ...
  // lx add
  //发布的最后一个参数为latch，设置为true，只有当订阅者链接时候才发布
  ...
  //服务端第一个参数 服务名称，第二个参数 
  // nh.advertiseService 返回一个SericeServer类型，推到容器中
...
/**
 * @brief 新增一个位姿估计器, 位姿估计器是imu和里程计的融合，这是做先验，前端匹配之前
 * 
 * @param[in] trajectory_id 轨迹id
...
void Node::AddExtrapolator(const int trajectory_id,
  ...
  // imu_gravity_time_constant在2d, 3d中都是10，在catorgrapher的配置文件中有写
...
void Node::LaunchSubscribers(const TrajectoryOptions& options,
  ...
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kLaserScanTopic, options.num_laser_scans))
  {
    //SubcribWithHandler是一个模板函数， 传入 一个函数指针，一个 id， 一个topic， 一个node_handle，一个node
    // 返回的是一个 ros::subscriber
    //subscribers 里边是一个 map 类型， value是 Subscriber结构体， 结构体默认构造 第一个是 ros::subscriber,第二个是topic

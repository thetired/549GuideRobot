    ...
    const nav_msgs::Odometry::ConstPtr& msg) 
{
  ...
  // 里程计的 child_frame_id是指 base_link 或者 foot_print
  ...
  // pose是 里程计坐标系下位姿， sensor_to_tracking 是 imu指向里程计的
    ...
    for (auto& observation : landmark_data.landmark_observations)
    {
      observation.landmark_to_tracking_transform =
          *tracking_from_landmark_sensor *
          observation.landmark_to_tracking_transform;  //catorgrapher，是相对于camera的，相对于camera坐标系的
    }
  }
  // 调用trajectory_builder_的AddSensorData进行数据的处理
    ...
    const carto::sensor::PointCloudWithIntensities& points) 
{
  ...
  // num_subdivision_per_laser_scan 是通过 lua文件传进来的
  // 如果为1 就进去一次
  for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) 
  {
    const size_t start_index =  //0
        points.points.size() * i / num_subdivisions_per_laser_scan_;
    const size_t end_index =    //100
    ...
    // 
    // time_to_subdivision_end时间是负的， 如果分半就减时间

...
NodeOptions CreateNodeOptions(
  ...
  // lua_parameter_dictionary->GetDictionary("map_builder") 生成一个智能指针，返回.get得到智能指针指向值
...
{
  // 获取配置文件所在的目录
   //vector只有一个元素 ，absl类似于stl
  auto file_resolver =
      absl::make_unique<cartographer::common::ConfigurationFileResolver>(
          std::vector<std::string>{configuration_directory});
        
  // 读取配置文件内容到code中，code把配置文件内容读了
  ...
  // 两部分，一部分赋予nodeOptions，另外一部分给trajectory options

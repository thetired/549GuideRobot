...
options = {
  ...
  tracking_frame = "imu_link",              -- 将所有传感器数据转换到这个坐标系下 有imulink就写，没有就写 base_link  footprint
  published_frame = "footprint",            -- tf: map -> footprint   最上边的坐标系
  ...
  -- 以上三者通过toplic来订阅，如果设置为true，要在launch文件中remap
  ...
  -- 采样频率 控制使用传感器频率，  可以降低计算量，抑制不太准数据

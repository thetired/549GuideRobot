...
{ // 第一次拷贝
  CHECK_NE(expected_sensor_ids_.count(sensor_id), 0);  //检查雷达话题名字是否为空

  // 从sensor_bridge传过来的数据的intensities为空 ，没有用
  ...
  if (id_to_pending_data_.count(sensor_id) != 0) 
  {
    ...
    // 保存当前点云  id_to_pending_data待处理的点云数据 ，当前这个点云还没用呢
    id_to_pending_data_.emplace(sensor_id, std::move(timed_point_cloud_data));
    return result; //start 和end时间内点云
  }

  // 先将当前点云添加到 等待时间同步的map中
  id_to_pending_data_.emplace(sensor_id, std::move(timed_point_cloud_data));

  // 等到range数据的话题都到来之后再进行处理 ，所有话题数据等待
...
sensor::TimedPointCloudOriginData RangeDataCollator::CropAndMerge() 
{
  ...
  {
    // 获取数据的引用
      //得到点云数据的range 和 intensities
    sensor::TimedPointCloudData& data = it->second;
    const sensor::TimedPointCloud& ranges = it->second.ranges;       //TimedPointCoudData 一帧点云
    const std::vector<float>& intensities = it->second.intensities;

    // 找到点云中 最后一个时间戳大于current_start_的点的索引
    // data.time 是 点云最后一个点时间

    auto overlap_begin = ranges.begin(); // [-6,-4,-2,-1 0]

    //点云最后一个点时间，+一个相对的时间，相对时间是负的
    ...
    // 找到点云中 最后一个时间戳小于等于current_end_的点的索引  ,前开后闭
    ...
    if (overlap_begin < overlap_end) 
    {
      ...
      //如果size为0 分配， 如果 size不等于0之前有值的 ，将之前数据拷贝到新内存中
  ...
  // 对各传感器的点云 按照每个点的时间从小到大进行排序，排序按lambda表达式规则排序

...
float ComputeCandidateScore(const ProbabilityGrid& probability_grid,
                            ...
                            int x_index_offset, int y_index_offset) {
  float candidate_score = 0.f;
  for (const Eigen::Array2i& xy_index : discrete_scan)  //每个点云的像素坐标
   {
    ...
    switch (grid.GetGridType()) {
      case GridType::PROBABILITY_GRID:
        candidate.score = ComputeCandidateScore(
            ...
            discrete_scans[candidate.scan_index],   // scan_index角度时候，点云经过预测平移处理后的点云
            candidate.x_index_offset,       //像素上x和y偏移多少

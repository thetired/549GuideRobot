...
class OccupiedSpaceCostFunction2D {
 public:
  ...
  bool operator()(const T* const pose, T* residual) const
   {
    Eigen::Matrix<T, 2, 1> translation(pose[0], pose[1]);  //得到平移
    Eigen::Rotation2D<T> rotation(pose[2]);                //得到旋转
    Eigen::Matrix<T, 2, 2> rotation_matrix = rotation.toRotationMatrix();
    Eigen::Matrix<T, 3, 3> transform;      //位姿变换矩阵
    ...
    for (size_t i = 0; i < point_cloud_.size(); ++i)
     {
      ...
      interpolator.Evaluate(
          (limits.max().x() - world[0]) / limits.resolution() - 0.5 +   //得到像素坐标
 ...
 private:
  ...
  class GridArrayAdapter {
   public:
    // 枚举 DATA_DIMENSION 表示被插值的向量或者函数的维度  ，必须存在的

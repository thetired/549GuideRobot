...
class LocalTrajectoryBuilder2D 
{
 ...
 private:
  ...
  ActiveSubmaps2D active_submaps_; //雷达数据不断往子图里添，子图完成就删掉，只保存两个子图

  MotionFilter motion_filter_;
  scan_matching::RealTimeCorrelativeScanMatcher2D   //之后讲
      real_time_correlative_scan_matcher_;
  scan_matching::CeresScanMatcher2D ceres_scan_matcher_; //之后讲

    ...
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) 
{
  ...
  // 只有slam刚启动时子图的个数才为1 ，刚开始建图
  if (insertion_submaps.size() == 1) 
  {
    // If we don't already have an entry for the first submap, add one.
    // 如果判断指定id的submap_data的size为0, 这条轨迹上还没有添加submap的pose
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0)  //等于0证明还没有submap pose 加入到优化问题里
    {
      // 如果没设置初始位姿就是0, 设置了就是1
      if (data_.initial_trajectory_poses.count(trajectory_id) > 0) {
        // 把该trajectory_id与其初始位姿的基准轨迹的id关联起来  ，不太重要之后说
    ...
    return {submap_id}; //返回的是vector
  }

  CHECK_EQ(2, insertion_submaps.size());

  // 获取 submap_data 的末尾 trajectory_id
  const auto end_it = submap_data.EndOfTrajectory(trajectory_id);  //前边创建了一个submap  id  这两个submap id 一样的
  ...
  if (data_.submap_data.at(last_submap_id).submap ==   //子图的指针和前边相等，证明子图后面还没加入优化问题里
    ...
    optimization_problem_->AddSubmap(  //子图pose加入
        trajectory_id,
  ...
  CHECK(data_.submap_data.at(last_submap_id).submap ==   //不等于前边那个，只能等于后边那个了
    ...
    const transform::Rigid3d& optimized_pose) 
{
  ...
  if (data_.submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0 ||
      std::prev(data_.submap_data.EndOfTrajectory(trajectory_id))   //EndofTrajectoryOrZero 是最后一个数据的下一个数据 prev 编程最后一个数据了
...
//花括号上锁
  {
    ...
    // 如果是未完成状态的地图不进行约束计算
    if (!data_.submap_data.at(submap_id).submap->insertion_finished())  //子图不是完成状态，赶紧退出，有点问题
    {
    ...
    //建图的时候始终是true，建图的时候回环检测只是结点与子图的局部匹配 ，纯定位的时候能不等
    if (node_id.trajectory_id == submap_id.trajectory_id ||
      ...
      maybe_add_local_constraint = true;    //根据时间来决定是匹配local 的 还是global的
    }
    // 如果节点与子图不属于同一条轨迹 并且 间隔了一段时间, 同时采样器为true
    // 才进行 全局搜索窗口 的约束计算(对整体子图进行回环检测)
    else if (global_localization_samplers_[node_id.trajectory_id]->Pulse())  //采样器是true的时候，才能global ，纯定位可能不计算约束
    {
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
  ...
  // 保存节点与计算子图内约束
  {
    ...
    for (size_t i = 0; i < insertion_submaps.size(); ++i) {
      ...
      data_.submap_data.at(submap_id).node_ids.emplace(node_id); //node_ids 是子图包含了哪些结点
      // 计算 子图原点 指向 node坐标 间的坐标变换(子图内约束)
      const transform::Rigid2d constraint_transform =
          constraints::ComputeSubmapPose(*insertion_submaps[i]).inverse() *
          local_pose_2d;
      // 新生成的 子图内约束 放入容器中
      // 约束是submap id  指向node  id的约束  ，权重是 pose graph传进来的权重 ，构建一个Constraint数据结构 ，这是一个循环
      //计算一个结点再两个子图内的约束
    ...
    if (newly_finished_submap) {
      const SubmapId newly_finished_submap_id = submap_ids.front(); //前边子图一定比后边子图先完成
  ...
  // Step: 当前节点与所有已经完成的子图进行约束的计算---实际上就是回环检测  ，计算比较多的
  //这一系列循环没有进行计算的，放入线程池里边了
  for (const auto& submap_id : finished_submap_ids) {
    // 计算旧的submap和新的节点间的约束
    // 拿着所有完成状态的id 与当前子图的id  匹配
    ComputeConstraint(node_id, submap_id); //回环检测
    //这部有好几层遍历嘛，每一次循环后 finish_node_task都会依赖一次约束计算 finish_node_task可能有1000多个依赖
  }

  // Step: 计算所有节点与刚完成子图间的约束---实际上就是回环检测
  // 内部结点不计算约束，这里是子图间的，子图内不计算
  if (newly_finished_submap)  //子图完成的时候去执行一次
  {
  ...
  absl::MutexLock locker(&mutex_);
  ++num_nodes_since_last_loop_closure_;  //上一次优化之后又有多少结点 ，也三个引用，执行完优化变量设为0了
  // Step: 插入的节点数大于optimize_every_n_nodes时执行一次优化
  // optimize_every_n_nodes = 0 时不进行优化, 这样就可以单独分析前端的效果
  if (options_.optimize_every_n_nodes() > 0 && // param: optimize_every_n_nodes   //这个设置成0就不优化了，如果想调前端的效果把这个设置成0，后端不起作用了
      num_nodes_since_last_loop_closure_ > options_.optimize_every_n_nodes()) {
    // 正在建图时只有这一块会返回 执行优化
    return WorkItem::Result::kRunOptimization;  //返回优化只有三次，三个可能， 两个是再结束的时候，只有这个是在建图的时候
  }
  return WorkItem::Result::kDoNotRunOptimization;
}
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
    ...
    const int trajectory_id) const 
  {
    //获取这个轨迹头部和尾部迭代器 , 找的是submap ，对于某个轨迹id，能不能找到submap
  ...
  if (begin_it == end_it) 
  {
    const auto it = data_.initial_trajectory_poses.find(trajectory_id);  //看一下这个轨迹id有没有初始位姿
    // 如果设置了初始位姿
    if (it != data_.initial_trajectory_poses.end()) {
      //这个TrajectoryID不是当前的id  ，initial_trajectory_pose 相对当前轨迹id的初始位姿，初始位姿坐标系，相对其他坐标系来表示的
      //设置的时候是SenInitalTrajectoryPose设置的

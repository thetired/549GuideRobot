...
/**
 * @brief 处理点云数据, 进行扫描匹配, 将点云写成地图,就是submap里，前端步骤
 * 
    ...
    const sensor::TimedPointCloudData& unsynchronized_data)
{
  ...
  // Step: 1 进行多个雷达点云数据的时间同步, 点云的坐标是相对于tracking_frame的
    //可以多个点云之前时间同步，也可以单个点云的时间同步
    // synchronized_data是同步后的点云
  ...
  // 如果不用imu, 就在雷达这初始化位姿推测器 ，如果用imu，在imu那初始化
  ...
  for (const auto& range : synchronized_data.ranges)
  {
    ...
    // Step: 2 预测出 每个点的时间戳时刻, tracking frame 在 local slam 坐标系下的位姿，位姿预测，之后讲
  ...
  // 对每个数据点进行处理
  for (size_t i = 0; i < synchronized_data.ranges.size(); ++i) //对每个点循环
  {
  ...
  if (num_accumulated_ >= options_.num_accumulated_range_data())
   {
    ...
    // 获取机器人当前姿态，相对启动slam时刻的姿态 ，，local下的
    ...
    return AddAccumulatedRangeData(
        ...
        TransformToGravityAlignedFrameAndFilter(
            gravity_alignment.cast<float>() * range_data_poses.back().inverse(),  //求一个坐标变换 ，最后一个点坐标 变换到local slam 原点坐标变换

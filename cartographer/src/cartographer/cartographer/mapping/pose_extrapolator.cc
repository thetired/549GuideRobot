...
// 使用imu数据进行PoseExtrapolator的初始化 //没有用
...
// 返回上次校准位姿的时间  time_pose_queue添加扫描匹配之后的位姿队列
common::Time PoseExtrapolator::GetLastPoseTime() const {
  // 如果尚未添加任何位姿, 则返回Time::min()
  if (timed_pose_queue_.empty()) {
    return common::Time::min();
  }
  return timed_pose_queue_.back().time;time
}
                               ...
                               const transform::Rigid3d& pose)
{
  ...
  // 保持pose队列中第二个pose的时间要大于 time - pose_queue_duration_ //修剪
  ...
  // 将imu_tracker_更新到time时刻，不断对imu数据更新
    ...
    const sensor::OdometryData& odometry_data)
{
  ...
  // 平移量除以时间得到 tracking frame 的线速度
  const Eigen::Vector3d
      linear_velocity_in_tracking_frame_at_newest_odometry_time =
          odometry_pose_delta.translation() / odometry_time_delta;

  // 根据位姿队列中最后一个位姿 乘以上次添加位姿时的姿态预测到time时刻的姿态变化量
  // 得到预测的 最新里程计数据时刻 tracking frame 在 local 坐标系下的姿态
  const Eigen::Quaterniond orientation_at_newest_odometry_time =
      timed_pose_queue_.back().pose.rotation() * // 位姿队列中上一时刻位姿
      ExtrapolateRotation(odometry_data_newest.time,
                          odometry_imu_tracker_.get());   // 上次添加位姿时的姿态预测到time时刻的姿态变化

...
ConfigurationFileResolver::ConfigurationFileResolver(
  ...
  //kConfigurationFilesDirectory 在 build_isolated 下的 common  下的 config.h 
  // 配置文件去  coniguration_files去找 在 install_isolated/share/cartographer
  //这就是为什么每次修改lua和launch文件需要重新编译 ，每次在这里边配置 install_isolated ，在src里边改luanch和lua文件没有用，需要重新遍历更新
  //这个config  由 config.h.cmake生成
}
